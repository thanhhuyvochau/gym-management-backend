...
import spring.project.base.config.vnpay.VnpConfig;
...
import spring.project.base.repository.TransactionRepository;
...
public class VnPayPaymentGateWay implements PaymentGateway<VnPayResponse> {
    ...
    @Override
    public PaymentResponse<VnPayResponse> pay(AppPlan appPlan) throws UnsupportedEncodingException {
        Transaction transaction = new Transaction();
        transaction.setAppPlan(appPlan);
        transaction.setAmount(appPlan.getPrice());
        ...
        PaymentResponse<VnPayResponse> paymentResponse = ConvertUtil.convertPaymentResponse(appPlan, transaction);
        paymentResponse.setPaymentData(new VnPayResponse(paymentUrl));
        return paymentResponse;
    }

    @NotNull
    private String buildPaymentUrl(HttpServletRequest req, Transaction transaction) throws UnsupportedEncodingException {
        Map<String, String> vnp_Params = prepareParameters(req, transaction.getAmount(),
    ...
    private Map<String, String> prepareParameters(HttpServletRequest req, BigDecimal price, String uniquePayCode) {
        ...
        String vnp_OrderInfo = "Thanh toán gói quản lý phòng gym";
